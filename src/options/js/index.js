...
const SETTINGS_PREFIX = 'browser-header-editor.settings';
let downloadUrl = '';
  ...
  if (rulesInput.value) {
    ...
    try {
      ...
      for (let item of rules) {
        if (
          Array.isArray(item.addRequestHeaders) &&
          item.addRequestHeaders.length
        ) {
          item.addRequestHeaders.forEach(function (item) {
            item.name = item.name.toLowerCase();
          });
        }
        if (
          Array.isArray(item.addResponseHeaders) &&
          item.addResponseHeaders.length
        ) {
          item.addResponseHeaders.forEach(function (item) {
            item.name = item.name.toLowerCase();
          });
        }
        if (
          Array.isArray(item.removeRequestHeaders) &&
          item.removeRequestHeaders.length
        ) {
          item.removeRequestHeaders = item.removeRequestHeaders
            .join()
            .toLowerCase()
            .split(',');
        }
        if (
          Array.isArray(item.removeResponseHeaders) &&
          item.removeResponseHeaders.length
        ) {
          item.removeResponseHeaders = item.removeResponseHeaders
            .join()
            .toLowerCase()
            .split(',');
        }
